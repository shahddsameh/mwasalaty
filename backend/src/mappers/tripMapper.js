...
function mapLeg(leg, itinIdx, legIdx) {
  ...
  return {
    ...
    route: leg.route
      ? { shortName: leg.route.shortName ?? null, longName: leg.route.longName ?? null }
      : null,
...
function pickHighlights(itineraries) {
  ...
  return {
    ...
    cheapest:     best(itineraries, (a, b) => a.totalFare.amount - b.totalFare.amount || a.durationMinutes - b.durationMinutes),
    leastWalking: best(itineraries, (a, b) => walkDistance(a) - walkDistance(b) || a.durationMinutes - b.durationMinutes),
  };
}
...
function sortItineraries(itineraries, strategy) {
  ...
  if (strategy === 'cheapest') {
    copy.sort((a, b) =>
      a.totalFare.amount - b.totalFare.amount || a.durationMinutes - b.durationMinutes
    );
...
export function mapOtpPlan(otpPlan, from, to, date, time, optimizedFor = 'quickest') {
  const mapped = deduplicateByPattern(
    otpPlan.itineraries.filter(itin => itin.legs.some(leg => leg.mode !== 'WALK'))
  ...
  if (optimizedFor === 'reliable' && sorted.length > 0) {
    const isFallback = !sorted.some(itin => itin.legs.some(l => l.mode === 'SUBWAY'));
