    ...
    <Card>
      ...
      >
        ...
        <div class="md:hidden divide-y divide-white/10">
          <div v-if="!loading && users.length === 0" class="p-8 text-center text-sm text-[#9CA3AF]">
            No users found.
          </div>
          <div
            v-for="user in paginatedUsers"
          ...
          >
            <div class="flex items-start justify-between gap-2">
              <div class="min-w-0 flex-1">
                <span class="text-xs font-semibold text-[#9CA3AF]">EMAIL</span>
                <p class="font-semibold text-white break-all mt-0.5 whitespace-normal">{{ user.email || "-" }}</p>
              </div>
              <div class="flex-shrink-0 text-right">
                <span class="text-xs font-semibold text-[#9CA3AF]">STATUS</span>
                <div class="mt-0.5">
                  <span
                    class="rounded-full px-2.5 py-0.5 text-xs font-medium inline-block"
                    :class="
                      user.status === 'blocked'
                        ? 'bg-red-500/10 text-[#EF4444]'
                        : 'bg-emerald-500/10 text-[#10B981]'
            ...
            <div class="grid grid-cols-2 gap-2 text-xs">
              <div>
                <span class="font-semibold text-[#9CA3AF]">NAME</span>
                <p class="text-[#94A3B8] truncate mt-0.5">{{ user.name || "-" }}</p>
              </div>
              <div>
                <span class="font-semibold text-[#9CA3AF]">PHONE</span>
                <p class="text-[#94A3B8] truncate mt-0.5">{{ user.phone || "-" }}</p>
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2 text-xs">
              <div>
                ...
                <p class="text-[#94A3B8] mt-0.5">{{ formatDate(user.created_at) }}</p>
              </div>
              <div>
                <span class="font-semibold text-[#9CA3AF]">LAST SIGN IN</span>
                <p class="text-[#94A3B8] mt-0.5">{{ formatDate(user.last_sign_in_at) }}</p>
              </div>
            </div>
            ...
            <div class="flex justify-end pt-2 border-t border-white/5">
              <button
                v-if="user.status !== 'blocked'"
                class="w-full sm:w-auto rounded-xl border border-red-500/30 px-3 py-2 font-semibold text-[#EF4444] hover:bg-red-500/10 transition-colors"
                :disabled="busyId === user.id"
                @click="block(user.id)"
              >
                Block
              ...
              <button
                v-else
                class="w-full sm:w-auto rounded-xl border border-emerald-500/30 px-3 py-2 font-semibold text-[#10B981] hover:bg-emerald-500/10 transition-colors"
                :disabled="busyId === user.id"
                @click="unblock(user.id)"
              >
                Unblock
