<template>
  ...
  >
    <div class="max-w-md w-full mx-auto py-8 md:py-12">
      <button
        v-if="!sent"
        class="flex items-center gap-2 text-muted-foreground hover:text-primary mb-6"
      ...
      >
        <template v-if="sent">
          ...
          <AppButton class="w-full" @click="router.push('/login')"
            >Back to Login</AppButton
          >
        </template>
        <template v-else>
