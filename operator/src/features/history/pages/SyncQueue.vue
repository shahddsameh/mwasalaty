  ...
  <main class="app-shell bottom-nav-offset">
    ...
    <section class="mx-auto grid w-full max-w-5xl gap-5">
      ...
      <p
        ...
        class="soft-alert border-warning bg-warning-soft text-warning"
      >
        <AppIcon name="warning" class="h-5 w-5 shrink-0" />{{
          ...
          >
            <div class="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <h2 class="mt-1 break-all text-base font-medium">
                  {{ scan.payload.ticketId }}
                </h2>
                <p class="mt-1 text-sm text-muted-foreground">
                  {{ scan.scannerProfileId }}
                </p>
              </div>
              <span
                :class="statusTone(scan)"
...
const discrepancies = computed(() =>
  scans.value.filter((scan) => scan.discrepancy),
);
const sortedScans = computed(() =>
  [...scans.value].sort(
    (a, b) =>
      Number(b.discrepancy) - Number(a.discrepancy) ||
...
const stateTitle = computed(() => {
  ...
  return discrepancies.value.length
    ? t("syncQueue.discrepancies", { count: discrepancies.value.length })
    : t("syncQueue.success");
});
const stateSupport = computed(() =>
  state.value === "ready" ? t("syncQueue.partial") : t("syncQueue.offline"),
);
...
function statusLabel(scan: QueuedScan) {
  if (scan.discrepancy && scan.reconciledOutcome === "already_used")
    return t("syncQueue.wasAlreadyUsed");
  if (scan.discrepancy && scan.reconciledOutcome === "invalid")
    return t("syncQueue.wasInvalid");
