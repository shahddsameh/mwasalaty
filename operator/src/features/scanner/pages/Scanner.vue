<template>
  <main class="app-shell bottom-nav-offset">
    ...
    <section class="mx-auto flex w-full max-w-5xl flex-col gap-4">
      ...
      <section class="relative min-h-[52dvh] overflow-hidden rounded-3xl border-4 border-white bg-slate-950 shadow-[0_18px_50px_rgba(15,23,42,0.18)] md:min-h-[60dvh]">
        ...
        <div v-if="status === 'live'" class="pointer-events-none absolute inset-0 scanner-grid">
          <div class="absolute inset-[10%] rounded-3xl border-2 border-primary shadow-[0_0_0_999px_rgba(2,6,23,0.48),0_0_30px_rgba(234,179,8,0.3)] sm:inset-[14%]">
            <span class="absolute -top-14 start-0 rounded-xl bg-slate-950/90 px-3 py-2 text-sm font-bold text-white shadow-lg backdrop-blur">
              {{ $t("scanner.empty") }}
            </span>
            <span class="absolute -left-1 -top-1 h-8 w-8 rounded-tl-3xl border-l-4 border-t-4 border-primary" />
            <span class="absolute -right-1 -top-1 h-8 w-8 rounded-tr-3xl border-r-4 border-t-4 border-primary" />
            <span class="absolute -bottom-1 -left-1 h-8 w-8 rounded-bl-3xl border-b-4 border-l-4 border-primary" />
            <span class="absolute -bottom-1 -right-1 h-8 w-8 rounded-br-3xl border-b-4 border-r-4 border-primary" />
          </div>
        </div>

        <div v-if="status !== 'live'" class="absolute inset-0 grid place-items-center bg-[radial-gradient(circle_at_top,#1e293b_0,#020617_65%)] p-6 text-center text-white">
          <div class="max-w-md">
            <div class="mx-auto mb-5 grid h-20 w-20 place-items-center rounded-3xl border border-white/10 bg-white/10 text-primary shadow-xl backdrop-blur" aria-hidden="true">
              <div v-if="status === 'starting'" class="h-9 w-9 animate-spin rounded-full border-4 border-white/20 border-t-primary" />
            ...
            <p v-if="status !== 'starting'" class="mx-auto mt-3 max-w-sm text-sm leading-6 text-slate-300">{{ status === 'denied' ? $t("scanner.denied") : $t("scanner.empty") }}</p>
            <div v-if="status !== 'starting'" class="mt-5 flex flex-wrap justify-center gap-3">
              ...
              <RouterLink to="/camera-help" class="tap-target inline-flex items-center justify-center rounded-xl border border-white/20 bg-white/10 px-4 py-2.5 font-bold text-white transition hover:bg-white/15 focus-ring">
                {{ $t("scanner.help") }}
              </RouterLink>
              <RouterLink to="/dashboard" class="tap-target inline-flex items-center justify-center rounded-xl px-4 py-2.5 font-bold text-slate-300 transition hover:text-white focus-ring">{{ $t("common.dashboard") }}</RouterLink>
            </div>
          </div>
        </div>

        <div v-if="processing" class="absolute inset-x-5 bottom-5 rounded-2xl border border-white/10 bg-slate-950/90 px-4 py-4 text-center text-lg font-black text-white shadow-xl backdrop-blur">
