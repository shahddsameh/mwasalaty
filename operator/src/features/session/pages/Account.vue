    ...
    <section class="mx-auto grid w-full max-w-5xl gap-5">
      <div v-if="isFallback" class="grid min-h-[70dvh] place-items-center">
        <div class="field-panel max-w-md p-7 text-center">
          <div
            class="mx-auto mb-5 grid h-16 w-16 place-items-center rounded-2xl bg-danger-soft text-destructive"
            aria-hidden="true"
          >
            <AppIcon name="warning" class="h-8 w-8" />
          </div>
          <h1 class="text-2xl font-semibold">{{ $t("account.fallback") }}</h1>
          <AppButton class="mt-5" size="lg" @click="reload">{{
            $t("account.reload")
          }}</AppButton>
        </div>
      </div>
      ...
      <template v-else>
        ...
        <dl class="grid gap-4 md:grid-cols-2">
          <div class="section-card">
            ...
            <dd class="mt-1 text-lg font-semibold">
              {{ profile?.operatorId ?? $t("common.dash") }}
            </dd>
          </div>
          <div class="section-card">
            <dt class="text-sm font-medium text-muted-foreground">
              {{ $t("account.selectedScanner") }}
            </dt>
            <dd class="mt-1 text-lg font-semibold">
          ...
          <div class="section-card">
            ...
            >
              <AppIcon name="language" class="h-5 w-5 text-primary-hover" />{{
                $t("account.language")
              }}
            </dt>
            <dd
              class="mt-3 grid grid-cols-2 rounded-xl border border-border bg-muted p-1"
            >
              <button
                type="button"
          ...
          <div class="section-card">
            <dt
              class="flex items-center gap-2 text-sm font-medium text-muted-foreground"
            >
              <AppIcon name="theme" class="h-5 w-5 text-primary-hover" />{{
                $t("account.theme")
              }}
            </dt>
            <dd
          ...
          <div class="section-card">
            ...
            <dd class="mt-1 text-xl font-semibold">{{ queuedCount }}</dd>
          </div>
          <div class="section-card">
            <dt class="text-sm font-medium text-muted-foreground">
              {{ $t("account.appVersion") }}
            </dt>
            <dd class="mt-1 font-mono text-lg font-medium">0.0.1</dd>
          </div>
        </dl>
        ...
        <section class="section-card grid gap-3 sm:grid-cols-2">
          <AppButton
            ...
            variant="outline"
            @click="changeScanner"
            ><AppIcon name="route" class="h-5 w-5" />{{
              $t("account.changeScanner")
            }}</AppButton
          >
          <AppButton
            class="w-full gap-2"
...
function signOut() {
  if (queuedCount.value > 0 && !window.confirm(t("account.signOutWarning")))
    return;
  clearSessionView();
  clearSelectedProfile();
