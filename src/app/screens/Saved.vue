<template>
  <main class="min-h-screen pb-20 bg-background">
    <div class="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-8 py-6 md:py-12">
      ...
      <section v-if="activeTab === 'places'">
        <div class="flex flex-col sm:flex-row justify-between gap-3 mb-6">
          ...
          <AppButton class="flex items-center gap-2" @click="addPlaceModalOpen = true">
      ...
      <section v-else-if="activeTab === 'ai'" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <SavedCard
      ...
      <section v-else class="space-y-4">
        <h2 class="font-display text-2xl text-foreground mb-6">Offline Routes</h2>
        <ListRow
    ...
    <Modal :open="addPlaceModalOpen" title="Add New Place" @close="addPlaceModalOpen = false">
      <div class="space-y-4">
        ...
        <input class="w-full px-4 py-2.5 bg-card border border-border rounded-lg" placeholder="Place name" />
        <input class="w-full px-4 py-2.5 bg-card border border-border rounded-lg" placeholder="Address or location" />
        <div class="flex gap-3">
          <AppButton class="flex-1" @click="addPlaceModalOpen = false">Save Place</AppButton>
          <AppButton variant="outline" class="flex-1" @click="addPlaceModalOpen = false">
            Cancel
          </AppButton>
        </div>
      </div>
...
import { savedPlaces } from "../data";

const router = useRouter();
const activeTab = ref<"places" | "routes" | "history" | "ai" | "offline">("places");
      ...
      h(
        ...
        [
          h("div", { class: "flex justify-between mb-4" }, [
            h(
              "div",
              { class: "w-12 h-12 rounded-lg bg-primary-soft flex items-center justify-center" },
          ...
          h("h3", { class: "font-display text-xl text-foreground mb-1" }, p.title),
          h("p", { class: "text-muted-foreground mb-3" }, p.subtitle),
          p.meta ? h("p", { class: "text-sm text-muted-foreground mb-4" }, p.meta) : null,
          h(AppButton, { variant: "outline", size: "sm", class: "w-full" }, () => "Open"),
        ],
      ),
});
    ...
    h(
      ...
      [
        h("div", { class: "flex flex-col sm:flex-row justify-between gap-4" }, [
          h("div", [
            h("h3", { class: "font-display text-xl text-foreground mb-2" }, p.title),
            h("p", { class: "text-muted-foreground mb-2" }, p.subtitle),
            p.meta ? h("p", { class: "text-sm text-muted-foreground" }, p.meta) : null,
          ]),
